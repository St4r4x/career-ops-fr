...
### Changed
- `frontend/components/settings/llm-providers-section.tsx` (new, replaces deleted `hf-token-section.tsx`) — Settings now shows a reorderable list of all 5 LLM providers (Hugging Face, Ollama Cloud, OpenAI, Anthropic, Groq) instead of a single Hugging Face token field; each row saves/deletes its own API key via `PUT`/`DELETE /api/settings/llm-providers/{provider}`, and configured providers can be reordered with ▲/▼ buttons via `PUT /api/settings/llm-providers/reorder`. Added `frontend/lib/llm-providers.ts` for the provider display-name map. `frontend/lib/types.ts`'s `SettingsResponse.hf_token_set: boolean` is now `llm_providers: LlmProvider[]`, and `OnboardingState.hf_token_complete` is now `llm_provider_complete`; `settings-client.tsx` and `onboarding-banner.tsx` updated to match
...
- `dashboard/api.py` — `GET /api/settings` now returns `llm_providers: [{provider, sort_order}]` instead of `hf_token_set: bool`; replaced `POST`/`DELETE /api/settings/hf-token` with `PUT`/`DELETE /api/settings/llm-providers/{provider}` (validates via `llm.validate_provider_key`, 404 on unknown provider name, empty `api_key` deletes) and `PUT /api/settings/llm-providers/reorder`; `POST /api/offers/{offer_id}/prepare`'s missing-provider check now looks at `user_data.get_llm_providers`/`get_llm_provider_key` instead of the single HF token (error code kept as `hf_token_missing` for frontend compatibility)
...
- `dashboard/scan_state.py` — per-user scan state (`start_scan()`/`get_scan_state()`), and `dashboard/api.py`'s `POST /api/scan/start`/`GET /api/scan/status`, replacing the old Jinja2 scan routes. Fixes a pre-existing bug where scan status was a single global dict shared across every user of the process instead of scoped per user
- `frontend/components/candidatures/scan-button.tsx` — scan trigger + live-progress polling (idle/running/done/error states, 2s poll via TanStack Query's `refetchInterval`), reproducing the former Jinja2 `scan_status.html` partial's behavior on the design system; refreshes the offers list automatically once a scan completes
...
- `scripts/scan_portals.py` — `list_portals_meta()`, returning `{id, name, status}` for every portal config, backing the new Settings portal picker
- `dashboard/api.py` — `GET /api/settings` now also returns `available_portals` (the portal registry), read-only, sourced from `portals/fr/*.yaml`
- `alembic/versions/0005_enabled_portals.py` — `user_settings.enabled_portals` (`text[]`), replacing `portal_queries`
...
- `proxy/nginx.conf` — added `location /settings` routing to `web`; the page is now fully served by the Next.js frontend. This was the last remaining Jinja2 page — the whole dashboard is now served by `web`
- `dashboard/api.py` — `_lang_query` FastAPI dependency, replacing 4 copies of an inline `lang` query-param normalization guard across the Profile CV mutation routes
- `frontend/components/profile/editable-section-header.tsx`, `text-edit-body.tsx` — shared header/body pieces extracted from the 7 Profile edit sections, replacing near-identical duplicated JSX across `contact-section.tsx`, `summary-section.tsx`, `cv-meta-section.tsx`, `cv-skills-section.tsx`, `cv-certifications-section.tsx`, `cv-education-section.tsx`, `cv-experience-section.tsx`
- `frontend/components/settings/hf-token-section.tsx` — switched from `invalidateQueries` to `setQueryData`, using the mutation's own `{hf_token_set}` response instead of forcing a full `/api/settings` refetch, matching `ats-targets-section.tsx`'s existing pattern
- `dashboard/user_data.py` — `get_cv()` now fetches all experience bullets in one batched `WHERE experience_id = ANY(%s)` query instead of one query per experience row
- `frontend/components/settings/search-settings-section.tsx` — replaced the freeform "Portal queries" textarea (a list of alternate search phrasings, unrelated to which portals get scraped despite the name) with a checkbox list of the actual scraped portals, tagging `needs_auth` ones with a "nécessite une inscription" badge
- `scripts/import_offers.py` — `_run_pipeline` now scans once per entry in `keywords` instead of the removed `portal_queries` list, which was a redundant, barely-used duplicate of the same setting
...
- `dashboard/scan_state.py` — `_run_scan`'s `import_offers`/`expire_stale_offers` calls are now wrapped in `asyncio.to_thread(...)`; a running scan was blocking the whole server's event loop like the `hf-token` and `prepare_state.py` bugs above
- `scripts/import_offers.py` — `_run_pipeline` read `settings.get("search", {})`, which is only ever populated when settings come from the `settings.yaml` file fallback; DB-backed settings (the normal path since the Settings page migration) are a flat dict, so every scan silently used the hardcoded `["AI Engineer"]` default instead of the user's configured keywords. Now uses `settings.get("search", settings)`, the same fallback `pre_filter.score_offer` already used for this same dual-shape settings dict
...
- `dashboard/app.py` — deleted the Jinja2-rendered `offer_edit_form`, `offer_save`, `offer_delete`, `offer_notes`, `offer_status` routes (`GET /offers/{id}/edit`, `POST /offers/{id}`, `DELETE /offers/{id}`, `POST /offers/{id}/notes`, `POST /offers/{id}/status`) now that the Candidatures detail panel's mutations are served by `PATCH`/`DELETE /api/offers/{id}` instead. `dashboard/templates/partials/offer_form.html` deleted with them. `partials/offer_detail.html` and `partials/offer_notes.html` are untouched — still rendered by `offer_prepare`, migrated in sub-phase D
...
- `dashboard/app.py`, `dashboard/templates/profile.html`, and 12 partials (`profile_contact.html`, `profile_text.html`, `profile_cv_meta.html`, `profile_cv_experience.html`, `profile_cv_skills.html`, `profile_cv_certifications.html`, `profile_cv_education.html`, plus the 5 pre-existing orphans `profile_education.html`/`profile_experience.html`/`profile_projects.html`/`profile_skills.html`/`profile_summary.html`) — deleted the Jinja2-rendered `/profile` route and its 8 mutation routes now that nginx routes `/profile` to `web`. `profile_summary.html` (referencing a nonexistent `/profile/summary` route) was a 5th orphan missed in sub-phase A's spec, found and deleted alongside the other 4
- `tests/test_profile_routes.py` — deleted, tested only the now-removed Jinja2 routes
- `dashboard/app.py`, `dashboard/templates/settings.html`, and 3 partials (`settings_search.html`, `settings_ats.html`, `settings_hf_token.html`) — deleted the Jinja2-rendered `/settings` route and its 5 mutation routes now that nginx routes `/settings` to `web`. This was the last remaining Jinja2 page: `dashboard/app.py` now contains zero page routes, only `app.include_router(api.router)` — its `Depends`/`Form`/`HTMLResponse`/`Jinja2Templates`/`CurrentUser`/`get_current_user`/`llm`/`user_data` imports and `TEMPLATES_DIR`/`templates` were all exclusively used by the routes just deleted, so they were removed too, along with `dashboard/templates/base.html` and `partials/onboarding_banner.html` (both now unreachable — nothing in the codebase instantiates `Jinja2Templates` anymore) and the now-empty `dashboard/templates/` directory tree
- `tests/test_dashboard_app.py` — deleted `TestOnboardingGateOnSettingsAndProfileNeverBlocks`, `TestSettingsHfTokenValidation`, `TestSettings`, the `authed_client` fixture, and `_DEFAULT_SETTINGS`/`_ATS_STORE`/`_HF_TOKEN_STORE` — tested only the now-removed Jinja2 routes
